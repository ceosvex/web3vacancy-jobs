---
title: "Graphic Designer at Stealth Project"
company: "Stealth Project"
location: "Office"
salary: "Competitive"
employment_type: "full-time"
date: "2026-06-01T14:01:37.210746+00:00"
permalink: "/jobs/graphic-designer-7312/"
description: "Graphic Designer at Stealth Project. We are looking for a Graphic Designer to create impactful advertising creatives and adapt designs for various…"
apply_url: "https://web3vacancy.com/job/graphic-designer-7312"
layout: job
---

# Graphic Designer at Stealth Project

**Company:** Stealth Project  
**Location:** Office  
**Salary:** Competitive  
**Type:** full-time

We are looking for a Graphic Designer to create impactful advertising creatives and adapt designs for various platforms. Join us to grow into a strong web/UI specialist.

**Skills:** `Figma` · `Photoshop` · `AI tools`

### Apply

**[→ View full details & apply on web3vacancy.com](https://web3vacancy.com/job/graphic-designer-7312)**

This role is listed on [web3vacancy.com](https://web3vacancy.com) — the curated job board for web3, crypto & blockchain careers.

